...
## Summary
This articles contained prediction analysis conducted on activiti monitoring to correctly predict activity being done based on measurement data. After some data processing, a number of features are selected and use to fit the provided data using random forest algorithm. Both the training and cross-validation has very high accuracy greater than 99.9% and the model gives perfect prediction on test data. This shows thet the devloped model can succesfully predict the type of activity being done with very high accuracy. 
